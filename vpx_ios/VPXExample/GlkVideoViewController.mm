...
NSTimeInterval system_up_time_ms() {
  return [[NSProcessInfo processInfo] systemUptime] *
      VpxExample::kMillisecondsPerSecond;
}

bool is_time_to_render(int64_t timestamp, NSTimeInterval start_time) {
  return (system_up_time_ms() - start_time) >= timestamp;
}
...
// Asynchronously tell fall back to the main UI and display |status| in the
// output text box. Must be dispatched to the main thread because this is
// ultimately a UI update.
...
- (void)playFile {
  self.videoStartTime = system_up_time_ms();
...
- (void)update {
  ...
  if ([_lock tryLock] == YES) {
    if (_videoFrames.empty()) {
      ...
      if (is_time_to_render(_videoFrames.front().timestamp_ms,
                            self.videoStartTime)) {
