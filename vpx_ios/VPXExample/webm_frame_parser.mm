...
bool WebmFrameParser::HasVpxFrames(const std::string &file_path,
  ...
  *vpx_format = vpx_format_;
  ...
  const int64_t ticks_per_nanosecond = segment_->GetInfo()->GetTimeCodeScale();
  const int64_t kExpectedWebmTimecodeScale = 1000000;
  if (ticks_per_nanosecond != kExpectedWebmTimecodeScale) {
    // TODO(tomfinegan): Handle arbitrary timecode scales.
    NSLog(@"Unsupported timecode scale in WebM file.");
    return false;
  }

  timebase_.numerator = ticks_per_nanosecond;
