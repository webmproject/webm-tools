...
print_cluster_pos_array() {
  ...
  for (( i=1; i < ${num_clusters}; i++ )); do
    # common.sh contains set -e; disable that temporarily since expr returns
    # 1 to the shell when an expression evaluates to 0.
    set +e
    last_array_index=$(expr $i - 1)
    set -e
    echo @\[@${cluster_start_pos_array[${last_array_index}]}, \
        @${cluster_start_pos_array[$i]}\],
  done
  set +e
  local last_cluster_index=$(expr ${num_clusters} - 1)
  set -e
