...
- (bool)periodMatches:(IxoDASHPeriod*)period
  ...
  for (NSString* const prop in props) {
    ...
    // Compare the prop values.
    if ([period valueForKey:prop] != [expectedPeriod valueForKey:prop]) {
      NSLog(@"-periodMatches:ExpectedPeriod %@ mismatch "
             "expected:%@ actual:%@",
            prop, [expectedPeriod valueForKey:prop], [period valueForKey:prop]);
    }
    XCTAssertEqual([period valueForKey:prop],
                   [expectedPeriod valueForKey:prop]);
  }
