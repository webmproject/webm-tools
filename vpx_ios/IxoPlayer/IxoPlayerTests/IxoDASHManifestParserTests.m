...
- (bool)representationMatches:(IxoDASHRepresentation*)rep
  ...
  for (NSString* const prop in props) {
    if ([rep valueForKey:prop] != [expectedRep valueForKey:prop]) {
      NSLog(@"-representationMatches:ExpectedRepresentation: %@ mismatch "
             "expected:%@ actual:%@",
            prop, [expectedRep valueForKey:prop], [rep valueForKey:prop]);
    }
...
- (bool)adaptationSetMatches:(IxoDASHAdaptationSet*)set
  ...
  for (NSString* const prop in props) {
    ...
    if ([set valueForKey:prop] != [expectedSet valueForKey:prop]) {
      NSLog(@"-adaptationSetMatches:ExpectedAdaptationSet: %@ mismatch "
             "expected:%@ actual:%@",
            prop, [expectedSet valueForKey:prop], [set valueForKey:prop]);
    }
...
- (bool)periodMatches:(IxoDASHPeriod*)period
  ...
  for (NSString* const prop in props) {
    ...
    // Compare the prop values.
    if (![self idIsStringType:[period valueForKey:prop]]) {
      XCTFail(@"Unknown type for value stored in IxoDASHPeriod");
      return false;
    }
...
- (bool)manifestMatches:(IxoDASHManifest*)manifest
  ...
  for (NSString* const prop in props) {
    ...
    if ([self idIsStringType:[manifest valueForKey:prop]]) {
      NSString* const value = [manifest valueForKey:prop];
      NSString* const expected_val = [expectedManifest valueForKey:prop];
      XCTAssertTrue([expected_val isEqualToString:value]);
    } else {
      // Compare the prop values.
      if ([manifest valueForKey:prop] != [expectedManifest valueForKey:prop]) {
        NSLog(@"-manifestMatches:expectedManifest: %@ mismatch "
               "expected:%@ actual:%@",
              prop, [expectedManifest valueForKey:prop],
              [manifest valueForKey:prop]);
      }
