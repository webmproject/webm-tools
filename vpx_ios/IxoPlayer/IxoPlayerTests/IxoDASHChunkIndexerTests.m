//
//  IxoDASHChunkIndexerTests.m
//  IxoPlayer
//
//  Created by Tom Finegan on 11/17/15.
//
//
...
- (void)testChunkIndexing {
  for (IxoDASHRepresentation* rep in
    ...
    IxoDASHStartData* start_data = [self fetchStartDataForRep:rep];
    XCTAssertNotNil(start_data);

    IxoDASHChunkIndexer* indexer =
        [[IxoDASHChunkIndexer alloc] initWithDASHStartData:start_data];
    XCTAssertNotNil(indexer);

    IxoDASHChunkIndex* index = [indexer buildChunkIndex];
