...
- (void)update {
  ...
  // TODO(tomfinegan): Add a check of screen width vs screen height to determine
  // whether or not the screen is in portrait mode before determining which
  // dimension to stretch. Note that this is not currently necessary because
  // VPXTest will not rotate to a portrait mode orientation.
  if (width >= height && video_aspect >= screen_aspect) {
