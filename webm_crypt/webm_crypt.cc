...
void TestEncryption() {
  ...
  string non_ascii;
  non_ascii.append(reinterpret_cast<char*>(raw_data), kNonAsciiSize);
  ...
  if (!key_file.empty()) {
    ...
    if (f) {
      ...
      if (bytes_read != key_size) {
        ...
        string raw_key;
        raw_key.append(raw_key_buf.get(), bytes_read);
...
void GenerateKeyFromContentID(const ContentEncoding* const encoding,
  ...
  if (encoding->GetEncryptionCount() > 0) {
    ...
    if (encryption->key_id_len > 0) {
      string raw_key;
      raw_key.append(reinterpret_cast<char*>(encryption->key_id),
                     static_cast<unsigned long>(encryption->key_id_len));
...
bool DecryptDataInPlace(const scoped_ptr<SymmetricKey>& key,
  ...
  encryptedtext.append(reinterpret_cast<const char*>(data), length);
...
bool EncryptDataInPlace(const scoped_ptr<SymmetricKey>& key,
  ...
  string plaintext;
  plaintext.append(reinterpret_cast<const char*>(data), length);
