...
bool DecryptModule::DecryptData(const uint8* data, size_t length,
  ...
  uint64 iv;
  const size_t offset = EncryptModule::kIntegrityCheckSize + sizeof(iv);
  if (!do_not_decrypt_) {
    memcpy(&iv, data + EncryptModule::kIntegrityCheckSize, sizeof(iv));
