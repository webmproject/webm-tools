...
static const char VERSION_STRING[] = "0.1.0.0";
...
class EncryptModule {
 public:
  ...
  // |enc| Encryption settings for a stream. |key| Encryption key for a stream.
  EncryptModule(const EncryptionSettings& enc, scoped_ptr<SymmetricKey>* key);
  ~EncryptModule();
  ...
  bool EncryptData(const unsigned char* data,
                   int length,
                   int64 time_ns,
                   string* ciphertext);
...
bool EncryptModule::EncryptData(const unsigned char* data,
                                int length,
                                int64 time_ns,
                                string* ciphertext) {
  if (!ciphertext)
...
bool EncryptModule::GenerateCounterBlock(const string& nonce,
  ...
  // Set the IV.
  memcpy(temp + kNonceSize, &iv, sizeof(iv));
...
class DecryptModule {
 public:
  ...
  DecryptModule(const EncryptionSettings& enc, scoped_ptr<SymmetricKey>* key);
  ~DecryptModule();
  ...
  // |decrypttext|.
  bool DecryptData(const unsigned char* data,
                   int length,
                   int64 time_ns,
                   string* decrypttext);
...
bool DecryptModule::DecryptData(const unsigned char* data,
                                int length,
                                int64 time_ns,
                                string* decrypttext) {
  if (!decrypttext)
...
bool OpenWebMFiles(const string& input,
  ...
  int64 ret = mkvparser::Segment::CreateInstance(reader,
                                                 pos,
                                                 *parser_segment);
  if (ret) {
...
// for a stream. |key| encryption key output parameter. Returns true on
// success.
bool GenerateKeyCheckFile(const EncryptionSettings& enc,
                          scoped_ptr<SymmetricKey>* key) {
  if (!key)
    return false;
  ...
  if (!enc.key_file.empty()) {
    ...
    key->reset(SymmetricKey::DeriveKeyFromPassword(
...
// Generates an encryption key from a WebM ContentID. Returns true on success.
bool GenerateKeyFromContentID(const ContentEncoding* encoding,
                              scoped_ptr<SymmetricKey>* key) {
  ...
  if (encoding->GetEncryptionCount() > 0) {
    ...
    if (encryption->key_id_len > 0) {
      ...
      scoped_ptr<SymmetricKey> sym_key(SymmetricKey::Import(SymmetricKey::AES,
                                                            raw_key));
      key->reset(SymmetricKey::Import(SymmetricKey::AES, raw_key));
    }
  }
...
// encryption key. |key| encryption key output parameter. Returns true on
// success.
...
int WebMEncrypt(const WebMCryptSettings& webm_crypt) {
  ...
  while (i != parser_tracks->GetTracksCount()) {
    ...
    if (track_type == kVideoTrack) {
      ...
      if (webm_crypt.video) {
        ...
        if (!GenerateKeyCheckFile(webm_crypt.vid_enc, &vid_key)) {
      ...
      if (webm_crypt.audio) {
        ...
        if (!GenerateKeyCheckFile(webm_crypt.aud_enc, &aud_key)) {
          printf("\n Could not generate encryption key.\n");
          return -1;
        }
  ...
  int data_len = 0;
  EncryptModule audio_encryptor(webm_crypt.aud_enc, &aud_key);
  EncryptModule video_encryptor(webm_crypt.vid_enc, &vid_key);
      ...
      if ((track_type == kAudioTrack) ||
        ...
        for (int i = 0; i < frame_count; ++i) {
          ...
          if (frame.len > data_len) {
            data.reset(new (std::nothrow) unsigned char[frame.len]);  // NOLINT
            if (!data.get())
              return -1;
            data_len = frame.len;
          }
  ...
  if (!webm_crypt.aud_enc.key_file.empty()) {
    ...
    if (!OutputKeyFile(webm_crypt.aud_enc, "aud_aes.key", raw_key)) {
      printf("Error writing to key file.\n");
      return -1;
    }
  }
  ...
  if (!webm_crypt.vid_enc.key_file.empty()) {
    ...
    if (!OutputKeyFile(webm_crypt.vid_enc, "vid_aes.key", raw_key)) {
      printf("Error writing to key file.\n");
      return -1;
    }
  }
...
int WebMDecrypt(const WebMCryptSettings& webm_crypt) {
  ...
  if (!webm_crypt.aud_enc.key_file.empty()) {
    if (!ReadKeyFromFile(webm_crypt.aud_enc.key_file, &aud_key))
      return -1;
  }

  scoped_ptr<SymmetricKey> vid_key;
  if (!webm_crypt.vid_enc.key_file.empty()) {
    if (!ReadKeyFromFile(webm_crypt.vid_enc.key_file, &vid_key))
      return -1;
  }
      ...
      if (encoding_count > 0) {
        ...
        if (vid_key.get() == NULL) {
          if (!webm_crypt.vid_enc.key_password.empty() &&
            ...
            // the ContentEncKeyID.
            GenerateKeyFromContentID(encoding, &vid_key);
          }
        }
      ...
      if (encoding_count > 0) {
        ...
        if (aud_key.get() == NULL) {
          if (!webm_crypt.aud_enc.key_password.empty() &&
            ...
            // TODO(fgalligan): Remove the code assuming the decryption key is
            // the ContentEncKeyID.
            GenerateKeyFromContentID(encoding, &aud_key);
          }
        }
  ...
  scoped_array<unsigned char> data;
  int data_len = 0;
  DecryptModule audio_decryptor(aud_enc, &aud_key);
  if (decrypt_audio && !audio_decryptor.Init()) {
    printf("\n Could not initialize audio decryptor.\n");
    return -1;
  }

  DecryptModule video_decryptor(vid_enc, &vid_key);
