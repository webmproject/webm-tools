...
class EncryptModule {
 public:
  ...
  // |enc| Encryption settings for a stream. |secret| Base secret to derive an
  // encryption key for a stream and is owned by this class.
  ...
  // encryptor whether to encrypt the frame or just add a signal byte to the
  // unencrypted frame. |ciphertext| is the returned potentially encrypted
  // data. Returns true if |data| was processed and passed back through
  // |ciphertext|.
 ...
 private:
  ...
  // String that holds the data to derive the encryption key.
