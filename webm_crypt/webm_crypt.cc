...
class EncryptModule {
 public:
  ...
  // Processes |plaintext| according to the encryption settings, encryption
  // key, and |encrypt_frame|. |length| is the size of |plaintext| in bytes.
 ...
 private:
  ...
  // String that holds the encryption key.
...
bool EncryptModule::Init() {
  key_.reset(SymmetricKey::Import(SymmetricKey::AES, secret_));
  if (!key_.get()) {
...
bool EncryptModule::ProcessData(const uint8* plaintext, size_t length,
  ...
  if (encrypt_the_frame) {
    ...
    ciphertext->assign(reinterpret_cast<const char*>(plaintext), length);
  }
...
class DecryptModule {
 public:
  ...
  // decrypted data if |data was decrypted. If data was unencrypted with a
  // signal byte then |decrypttext| is the original plaintext minus the signal
  // byte. Returns true if |data| was decrypted and passed back through
  // |decrypttext|.
 ...
 private:
  ...
  // String that holds the decryption key.
  const string secret_;
