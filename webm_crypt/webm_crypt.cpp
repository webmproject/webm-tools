...
void GenerateKeyCheckFile(const string& key_file,
  ...
  if (!key_file.empty()) {
    ...
    if (f) {
      ...
      char* const raw_key_buf = const_cast<char*>(raw_key.data());
...
void GenerateKeyFromContentID(const ContentEncoding* const encoding,
  ...
  if (encoding->GetEncryptionCount() > 0) {
    ...
    if (encryption->key_id_len > 0) {
      string raw_key;
      raw_key.resize(static_cast<unsigned int>(encryption->key_id_len));
      char* const raw_key_buf = const_cast<char*>(raw_key.data());
      memcpy(raw_key_buf,
             encryption->key_id,
...
bool DecryptDataInPlace(const scoped_ptr<SymmetricKey>& key,
  ...
  char* const raw_buf = const_cast<char*>(encryptedtext.data());
...
bool EncryptDataInPlace(const scoped_ptr<SymmetricKey>& key,
  ...
  char* const raw_buf = const_cast<char*>(plaintext.data());
  memcpy(raw_buf, data, length);
      ...
      if ((track_type == kAudioTrack) ||
        ...
        for (int i = 0; i < frame_count; ++i) {
          ...
          if ( (track_type == kVideoTrack && encrypt_video) ||
            ...
            unsigned char* const encrypted_data =
      ...
      if ((track_type == kAudioTrack) ||
        ...
        for (int i = 0; i < frame_count; ++i) {
          ...
          if ( (track_type == kVideoTrack && decrypt_video) ||
            ...
            unsigned char* const decrypted_data =
                reinterpret_cast<unsigned char*>(
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (!strcmp("-h", argv[i]) || !strcmp("-?", argv[i])) {
      ...
      audio = (!strcmp("true", argv[++i])) ? true : false;
    } else if (!strcmp("-video", argv[i]) && i < argc_check) {
      video = (!strcmp("false", argv[++i])) ? false : true;
  ...
  if (encrypt) {
    int rv = WebMEncrypt(input, output, key_file, video, audio, &content_id);
    if (rv < 0) {
