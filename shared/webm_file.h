...
class WebMFile {
 public:
  ...
  // Returns true if the CuePoints across |webm_list| are aligned. |seconds| is
  // the range in seconds that the function is allowed to search for
  // alignment. I.e. If file A had a CuePoint every 5 seconds and file B had
  // a CuePoint every 15 seconds then the files would be aligned if
  // |seconds| >= 15.0. |check_for_sap| if true checks if the aligned
  // CuePoints start with a key frame. |check_for_audio_match| if true checks
  // that the first audio block in aligned CuePoints are the same. |verbose|
  // if true outputs more information to stdout. |output_alignment_times| if
  // true will append the aligned CuePoiunt times in seconds to
  // |output_string|. |output_alignment_stats| if true will append the
  // aligned CuePoiunt times in seconds and the reason if aligned CuePoints
  // were rejected because of |check_for_sap| or |check_for_audio_match|.
  // |output_alignment_stats| supersedes |output_alignment_times|.
  // |output_string| is an output parameter with information on why the
  // function returned false and/or the output from |output_alignment_stats|
  // or |output_alignment_times|.
  static bool CheckCuesAlignementList(
  ...
  // the Cluster for that track. |error_nano| is an output parameter of the
  // first time encountered that a CuePoint was not the first Block in a
  // cluster of |type|. |error_nano| may be NULL. |error_nano| will only be
  // set if the function returns false.
  bool CuesFirstInCluster(TrackTypes type, int64* error_nano) const;
 ...
 private:
  ...
  // the frame rate.
  double CalculateFrameRate(int64 track_number) const;
  ...
  // find a Block of |track_num| within the Cluster.
  ...
  // Populates the CueDesc list from the Cues element. Returns true on success.
  ...
  // Returns true if the first Block of |track| within the Cluster is a key
  // frame. |cp| is the CuePoint that references the Cluster to search.
  // |track| the Track to look for the first Block. |error_nano| is the time
  // of the first Block that is set if the function returns false.
  // |error_nano| may be NULL. |frame_is_altref| is set to true if the first
  // Block is a VP8 altref frame. |frame_is_altref| may be NULL.
