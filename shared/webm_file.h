...
// |output_alignment_times|. |output_string| may be NULL.
  ...
  // per second.
  int64 FileAverageBandwidth() const;

  // Calculate and return average bandwidth for the WebM file in bits per
  // second.
  int64 FileAverageBitrate() const;
  ...
  // per second.
  int64 FileMaximumBandwidth() const;
  ...
  // Returns average bandwidth for the first track of track |type| in bits per
  // second. Returns 0 on error.
  int64 TrackAverageBitrate(TrackTypes type) const;
 ...
 private:
  // Calculate and return average bandwidth for the WebM file in bits or
  // kilobits per second starting from |cp|. If |cp| is NULL calculate the
  // bandwidth over the entire file. If |kilobits| is true the return value
  // will be in kilobits per second otherwise the return value will be in bits
  // per second. Returns 0 on error.
  int64 CalculateBandwidth(const mkvparser::CuePoint* cp, bool kilobits) const;
  ...
  // Returns average bandwidth for |track_number| in bits or kilobits per
  // second starting from |cp|. If |cp| is NULL calculate the bandwidth over
  // the entire file. If |kilobits| is true the return value  will be in
  // kilobits per second otherwise the return value will be in bits per second.
  // Returns 0 on error.
  int64 CalculateTrackBandwidth(int track_number,
