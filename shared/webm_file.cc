...
bool WebMFile::Init() {
  ...
  const string doc_type(ebml_header.m_docType);
  if (!CheckDocType(doc_type)) {
    ...
    for (unsigned int i = 0; i < cues_list_size; ++i) {
      ...
      if (!cues->Find(nano, track, cp, tp)) {
        ...
        if (output_string) {
          char str[1024];
          snprintf(str, sizeof(str),
                   "Could not find CuePoint time:%lld track:%lld file:%s",
      ...
      if (cp->GetTimeCode() > no_alignment_timecode) {
        ...
        if (output_string) {
          ...
          snprintf(str, sizeof(str),
                   ...
                   " last_alignment:%lld cp time:%lld track:%lld file:%s",
      ...
      if (cp->GetTimeCode() != time) {
        const WebMFile* const file = webm_list.at(i);
        if (verbose) {
          printf("No alignment found at time:%lld cp time:%lld track:%lld"
                 " file:%s\n",
    ...
    if (found_alignment && check_for_sap) {
      for (unsigned int i = 0; i < cues_list.size(); ++i) {
        ...
        if (!file->StartsWithKey(*cp, *cluster, *block)) {
          ...
          char str[4096];
          snprintf(str, sizeof(str),
                   " |!Key nano:%lld sec:%g altref:%s track:%lld file:%s",
...
int64 WebMFile::PeakBandwidthOverFile(int64 prebuffer_ns) const {
  const mkvparser::Cues* const cues = GetCues();
  if (!cues)
    return 0;
...
double WebMFile::CalculateFrameRate(int64 track_number) const {
...
int WebMFile::PeakBandwidth(int64 time_ns,
                            int64 prebuffer_ns,
  ...
  if (!desc_beg) {
    fprintf(stderr,
            "PeakBandwidth() GetCueDescFromTime returned NULL. time_ns:%lld\n",
            time_ns);
    return -1;
  }
  ...
  if (desc_beg->start_time_ns != time_ns) {
    fprintf(stderr,
            "PeakBandwidth() CueDesc time != time_ns. time:%lld"
            " time_ns:%lld\n",
  ...
  *bandwidth = 0.0;
  do {
    ...
    if (prebuffer < desc_sec) {
      ...
      if (rv < 0) {
        fprintf(stderr,
                "PeakBandwidth() BufferSizeAfterTimeDownloaded rv:%d\n", rv);
        return rv;
      } else if (rv == 0) {
