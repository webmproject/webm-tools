...
class WebMFile {
 public:
  explicit WebMFile(const string& filename);
  ~WebMFile();
  ...
  // seconds will be added to the valuer passed into |buffer|. |sec_counted|
  ...
  // time in seconds will be added to the valuer passed into |buffer|.
  // |sec_to_download| is the time in seconds that it took to download the
  // data.
  // Return values < 0 are errors. Return value of 0 is success.
  // Return value of 1 is the walk had a buffer underrun.
  ...
  string GetMimeTypeWithCodec() const;
