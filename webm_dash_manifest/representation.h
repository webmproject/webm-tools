...
class Representation {
 public:
  Representation(const string& id, const DashModel& dash);
  ~Representation();
  ...
  bool OutputDashManifest(std::ostream& o, indent_webm::Indent& indt) const;
  ...
  const string& webm_filename() const { return webm_filename_; }
  void set_webm_filename(const string& file) { webm_filename_ = file; }

 private:
  // Outputs SegmentBase in the Dash format. Returns true if there were no
  // errors with the output.
  bool OutputSegmentBase(std::ostream& o, indent_webm::Indent& indt) const;
  ...
  // Flag telling if the the class should output audio sample rate.
  bool output_audio_sample_rate_;

  // Flag telling if the the class should output the header information.
  bool output_header_;

  // Flag telling if the the class should output the header information.
  bool output_index_;

  // Flag telling if the the class should output video height.
  bool output_video_height_;

  // Flag telling if the the class should output video width.
