...
bool DashModel::Init() {
  ...
  for(period_iter = periods_.begin();
      period_iter != periods_.end();
      ++period_iter) {
    for (int i = 0; i < (*period_iter)->AdaptationSetIDSize(); ++i) {
      string id;
      if (!(*period_iter)->AdaptationSetID(i, &id))
        return false;
      ...
      (*period_iter)->AddAdaptationSet(as);
    }
  }
...
bool DashModel::OutputDashManifestFile() const {
  ...
  FILE* o = fopen (output_filename_.c_str() , "w");
  if (!o)
    return false;
