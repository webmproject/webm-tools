...
bool WebMFile::Init() {
  ...
  if (reader_->Open(filename_.c_str())) {
    cout << "Error trying to open file:" << filename_ << endl;
    return false;
  }
  ...
  if (!LoadCueDescList()) {
    cout << "LoadCueDescList() failed." << endl;
    return false;
  }
...
double WebMFile::CalculateFrameRate(long long track_number) const {
  assert(segment_->GetInfo()->GetDuration());
...
void WebMFile::FindCuesChunk(long long start_time_nano,
 ...
 if (!start || !end || !cue_start_time || !cue_end_time)
   return;
...
int WebMFile::PeakBandwidth(long long time_ns,
  ...
  if (!desc_beg) {
    cout << "PeakBandwidth() GetCueDescFromTime returned NULL. time_ns:"
         << time_ns << endl;
    return -1;
  }
  ...
  if (desc_beg->start_time_ns != time_ns) {
    cout << "PeakBandwidth() CueDesc time != time_ns. time:"
         << desc_beg->start_time_ns << " times_ns:" << time_ns << endl;
    return -1;
  }
  ...
  if (!desc_end) {
    ...
    *bandwidth = 0.0;
    assert(segment_->GetInfo()->GetDuration() < prebuffered_ns);
    return 0;
  }
  ...
  do {
    ...
    if (prebuffer < desc_sec) {
      ...
      const int rv = BufferSizeAfterTimeDownloaded(prebuffered_ns,
                                                   ...
                                                   &sec_to_download);
      if (rv < 0) {
        cout << "PeakBandwidth() BufferSizeAfterTimeDownloaded rv:" << rv
             << endl;
        return rv;
      } else if (rv == 0) {
