...
int WebMFile::BufferSizeAfterTime(double time,
  ...
  const int64 time_ns = static_cast<int64>(time * 1000000000.0);
...
int WebMFile::BufferSizeAfterTimeDownloaded(int64 time_ns,
  ...
  const int64 time_to_search_ns =
      static_cast<int64>(search_sec * 1000000000.0);
...
int64 WebMFile::CalculateBandwidth(const mkvparser::CuePoint* cp) const {
  ...
  const int64 bandwidth =
    static_cast<int64>((filesize * 8) / (duration / 1000000000.0) / 1000);
  return bandwidth;
}
