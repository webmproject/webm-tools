...
static int read_frame(FILE *f, vpx_image_t *img, vpx_image_t *img_alpha) {
  ...
  for (i = 0; i < 4; i++) {
    const int width = (i == 0 || i == 3) ? w : uv_w;
    const int height = (i == 0 || i == 3) ? h : uv_h;
...
typedef enum stereo_format {
  kStereoFormatMono      = 0,
  kStereoFormatLeftRight = 1,
  kStereoFormatBottomTop = 2,
  kStereoFormatTopBottom = 3,
  kStereoFormatRightLeft = 11
} stereo_format_t;
 ...
 */
static void Ebml_SerializeUnsigned32(EbmlGlobal *glob, unsigned long class_id,
                                     uint64_t ui) {
...
static void Ebml_EndSubElement(EbmlGlobal *glob, EbmlLoc *ebmlLoc) {
  ...
  const off_t pos = ftello(glob->stream);
...
static void write_webm_seek_element(EbmlGlobal *ebml, unsigned long id,
                                    off_t pos) {
...
static void write_webm_seek_info(EbmlGlobal *ebml) {
  ...
  }
  {
    ...
    Ebml_StartSubElement(ebml, &start_info, Info);
    Ebml_SerializeUnsigned(ebml, TimecodeScale, 1000000);
    Ebml_SerializeFloat(ebml, Segment_Duration,
                        (double)(ebml->last_pts_ms + frame_time));
    Ebml_SerializeString(ebml, 0x4D80, version_string);
    Ebml_SerializeString(ebml, 0x5741, version_string);
    Ebml_EndSubElement(ebml, &start_info);
  }
}
...

    {
      ...
      Ebml_StartSubElement(glob, &trackStart, Tracks);
      {
        ...
        Ebml_SerializeString(glob, CodecID, "V_VP8");
        {
          ...
          Ebml_SerializeUnsigned(glob, PixelWidth, cfg->g_w);
          Ebml_SerializeUnsigned(glob, PixelHeight, cfg->g_h);
...
static void write_webm_block(EbmlGlobal *glob,
  ...
  int is_keyframe;
  EbmlLoc block_group;
  EbmlLoc block_additions;
  EbmlLoc block_more;

  /* Calculate the PTS of this frame in milliseconds */
  pts_ms = pkt->data.frame.pts * 1000 * (uint64_t)cfg->g_timebase.num /
...
int main(int argc, char **argv) {
  ...
  /* write header*/
  write_webm_file_header(&ebml, &cfg, &frame_rate, kStereoFormatMono);
  ...
  while (frame_avail || got_data) {
    ...
    if (vpx_codec_encode(&codec, frame_avail? &raw : NULL, frame_cnt, 1,
    ...
    while ((pkt = vpx_codec_get_cx_data(&codec, &iter)) &&
      ...
      switch (pkt->kind) {
        case VPX_CODEC_CX_FRAME_PKT:
          ...
          hash = murmur(pkt->data.frame.buf, (int)pkt->data.frame.sz, hash);
          break;
