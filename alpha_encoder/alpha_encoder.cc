...
bool create_input_files(char* input, int w, int h) {
  FILE* f_input;
  FILE* f_video;
  FILE* f_alpha;
  ...
  mkvmuxer::uint8* rows[6];

  for (int i = 0; i < 6; i++)
    rows[i] = new unsigned char[(i == 0 || i == 3) ? w : uv_w];

  // TODO(vigneshv): Should this be 0 or something else ?
  memset(rows[4], 0, uv_w);
  memset(rows[5], 0, uv_w);

  bool eof = false;
  while (!eof) {
    for (int i = 0; i < 6 && !eof; ++i) {
      const mkvmuxer::uint32 width = (i == 0 || i == 3) ? w : uv_w;
      const mkvmuxer::uint32 height = (i == 0 || i == 3) ? h : uv_h;
      for (unsigned int row = 0; row < height; row++) {
        if (i < 4) {
          if (fread(rows[i], 1, width, f_input) != width) {
            eof = true;
            break;
          }
        }
        fwrite(rows[i], 1, width, (i < 3) ? f_video : f_alpha);
      }
    }
  }
...
bool encode(int w, int h,
            std::string vpxenc_cmd,
            std::string vpxenc_options) {
  char encode_cmd[10000];
  snprintf(encode_cmd,
           10000,
           "%s --width=%d --height=%d %s -o video.out video.in",
           vpxenc_cmd.c_str(), w, h, vpxenc_options.c_str());
  fprintf(stderr, "Running %s\n", encode_cmd);
  if (system(encode_cmd))
    return false;
  snprintf(encode_cmd,
           10000,
           "%s --width=%d --height=%d %s -o alpha.out alpha.in",
           vpxenc_cmd.c_str(), w, h, vpxenc_options.c_str());
  fprintf(stderr, "Running %s\n", encode_cmd);
  if (system(encode_cmd))
    return false;
...
void Usage() {
  printf("Usage: alpha_encoder -i input -o output -h height -w width -v "
         "vpexenc_cmd [vpxenc_options]\n");
  printf("Options:\n");
...
bool init(mkvmuxer::MkvWriter* writer,
          char *output,
...
bool write_track(mkvparser::MkvReader* reader,
                 mkvparser::MkvReader* reader_alpha,
                 mkvmuxer::Segment* muxer_segment,
                 mkvparser::Segment** parser_segment,
                 mkvparser::Segment** parser_segment_alpha) {
...
bool write_clusters(mkvparser::MkvReader* reader,
                    mkvparser::MkvReader* reader_alpha,
                    mkvmuxer::Segment* muxer_segment,
                    mkvparser::Segment* parser_segment,
                    mkvparser::Segment* parser_segment_alpha) {
...
int main(int argc, char** argv) {
  ...
  if (!create_input_files(input, w, h) ||
      !encode(w, h, vpxenc_cmd, vpxenc_options) ||
      !init(&writer, output, &reader, &reader_alpha, &muxer_segment) ||
      !write_track(&reader, &reader_alpha, &muxer_segment,
                   &parser_segment, &parser_segment_alpha) ||
      !write_clusters(&reader, &reader_alpha, &muxer_segment,
                      parser_segment, parser_segment_alpha) ||
