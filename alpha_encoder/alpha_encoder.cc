...
bool CreateInputFiles(const char* input, int w, int h) {
  ...
  FILE* const f_video = fopen("video.in", "w");
  if (!f_video) {
    fprintf(stderr, "Failed to open video.in for write");
    return false;
  }
  FILE* const f_alpha = fopen("alpha.in", "w");
  if (!f_alpha) {
  ...
  for (int i = 0; i < 6; i++)
    rows[i] = new uint8[(i == 0 || i == 3) ? w : uv_w];
  ...
  while (!eof) {
    for (int i = 0; i < 6 && !eof; ++i) {
      const uint32 width = (i == 0 || i == 3) ? w : uv_w;
      const uint32 height = (i == 0 || i == 3) ? h : uv_h;
...
int main(int argc, char** argv) {
  ...
  if (!CreateInputFiles(input, w, h) ||
      ...
      !Init(output, &writer, &reader, &reader_alpha, &muxer_segment) ||
