...
Options::Options()
    : output_video(true),
      ...
      output_blocks(true),
      output_vp8_info(true),
      output_clusters_size(true) {
}
...
void Usage() {
  ...
  printf("Main options:\n");
  printf("  -h | -?                     show help\n");
  printf("  -video <bool>               Output video tracks.\n");
  printf("  -audio <bool>               Output audio tracks.\n");
  printf("  -size <bool>                Output element sizes.\n");
  printf("  -offset <bool>              Output element offsets.\n");
  printf("  -ebml_header <bool>         Output EBML header.\n");
  printf("  -segment <bool>             Output Segment.\n");
  printf("  -segment_info <bool>        Output SegmentInfo.\n");
  printf("  -tracks <bool>              Output Tracks.\n");
  printf("  -clusters <bool>            Output Clusters.\n");
  printf("  -blocks <bool>              Output Blocks.\n");
  printf("  -vp8_info <bool>            Output VP8 information.\n");
  printf("  -clusters_size <bool>       Output Total Clusters size.\n");
}
...
void OutputEBMLHeader(FILE* o,
                      const mkvparser::EBMLHeader& ebml,
                      Indent* indent) {
...
bool OutputSegmentInfo(FILE* o,
  ...
  fprintf(o, "%sDuration(secs): %g\n",
          indent->indent_str().c_str(), duration_ns / 1000000000.0);
...
bool OutputCluster(FILE* o,
  ...
  if (options.output_clusters) {
    const int64 timecode = cluster.GetTimeCode();
    ...
    fprintf(o, "\n");
    fprintf(o, "%sTimecode       : %lld\n",
            indent->indent_str().c_str(), timecode);
    fprintf(o, "%sTimecode (nano): %lld\n",
            indent->indent_str().c_str(), time_ns);
    fprintf(o, "%sTimecode (sec) : %g\n",
            indent->indent_str().c_str(), time_ns / 1000000000.0);
  ...
  if (options.output_blocks) {
    ...
    while (block_entry != NULL && !block_entry->EOS()) {
      ...
      if ((track_type == VIDEO_TRACK && options.output_video) ||
        ...
        fprintf(o, "%sBlock: type:%s frame:%s nano:%10lld secs:%5g",
                indent->indent_str().c_str(),
                track_type == VIDEO_TRACK ? "V" : "A",
                is_key ? "I" : "P",
                time_ns,
                time_ns / 1000000000.0);
        if (options.output_offset)
          fprintf(o, " @_p: %lld", block->m_start);
        if (options.output_size)
          fprintf(o, " size_p: %lld", block->m_size);
        ...
        if (options.output_vp8_info) {
          ...
          for (int i = 0; i < frame_count; ++i) {
            if (track_type == VIDEO_TRACK) {
              ...
              if (frame.len > data_size) {
                delete [] data;
                data_size = frame.len + 1024;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (!strcmp("-h", argv[i]) || !strcmp("-?", argv[i])) {
      ...
      options.output_video = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-audio", argv[i]) && i < argc_check) {
      options.output_audio = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-size", argv[i]) && i < argc_check) {
      options.output_size = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-offset", argv[i]) && i < argc_check) {
      options.output_offset = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-ebml_header", argv[i]) && i < argc_check) {
      options.output_ebml_header =
          strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-segment", argv[i]) && i < argc_check) {
      options.output_segment = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-segment_info", argv[i]) && i < argc_check) {
      options.output_segment_info =
          strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-tracks", argv[i]) && i < argc_check) {
      options.output_tracks = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-clusters", argv[i]) && i < argc_check) {
      options.output_clusters = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-blocks", argv[i]) && i < argc_check) {
      options.output_blocks = strcmp("false", argv[++i]) == 0 ? false : true;
    } else if (!strcmp("-vp8_info", argv[i]) && i < argc_check) {
      options.output_vp8_info = strcmp("false", argv[++i]) == 0 ? false : true;
    }
  }
  ...
  std::auto_ptr<mkvparser::MkvReader> reader(new mkvparser::MkvReader());
  ...
  std::auto_ptr<mkvparser::EBMLHeader>
      ebml_header(new mkvparser::EBMLHeader());
  ...
  if (options.output_ebml_header)
    OutputEBMLHeader(out, *ebml_header.get(), &indent);
  ...
  if (options.output_segment) {
    OutputSegment(out, *(segment.get()), options);
    indent.Adjust(webm_tools::kIncreaseIndent);
  }

  if (options.output_segment_info)
    if (!OutputSegmentInfo(out, *(segment.get()), options, &indent))
      return EXIT_FAILURE;

  if (options.output_tracks)
    if (!OutputTracks(out, *(segment.get()), options, &indent))
      return EXIT_FAILURE;
